...
class MGPCGSolver:
    ...
    def gridtype_init(self, l: ti.template()):
        for i, j in self.grid_type[l]:
            ...
            #     self.grid_type[l][i, j] = 2

            i2 = i * 2
            j2 = j * 2

            if self.grid_type[l - 1][i2, j2] == 0 or self.grid_type[
                    l - 1][i2, j2 + 1] == 0 or self.grid_type[l - 1][
                        i2 + 1,
                        j2] == 0 or self.grid_type[l -
                                                           1][i2 + 1, j2 +
                                                              1] == 0:
                self.grid_type[l][i, j] = 0
            else:
                if self.grid_type[l - 1][
                        i2, j2] == 1 or self.grid_type[l - 1][
                            i2,
                            j2 + 1] == 1 or self.grid_type[l - 1][
                                i2 + 1, j2] == 1 or self.grid_type[
                                    l - 1][i2 + 1, j2 + 1] == 1:
                    self.grid_type[l][i, j] = 1
                else:
                    self.grid_type[l][i, j] = 2

    @ti.kernel
    def preconditioner_init(self, old_scale: ti.f32, l: ti.template()):
        scale = old_scale / (2**l * 2**l)

        for i, j in self.grid_type[l]:
            if self.grid_type[l][i, j] == 1:
                if self.grid_type[l][i - 1, j] == 1:
                    self.Adiag[l][i, j] += scale
                if self.grid_type[l][i + 1, j] == 1:
                    self.Adiag[l][i, j] += scale
                    self.Ax[l][i, j] = -scale
                elif self.grid_type[l][i + 1, j] == 0:
                    self.Adiag[l][i, j] += scale

                if self.grid_type[l][i, j - 1] == 1:
                    self.Adiag[l][i, j] += scale
                if self.grid_type[l][i, j + 1] == 1:
                    self.Adiag[l][i, j] += scale
                    self.Ay[l][i, j] = -scale
                elif self.grid_type[l][i, j + 1] == 0:
                    self.Adiag[l][i, j] += scale
    ...
    def smooth(self, l: ti.template(), phase: ti.i32):
        ...
        for i, j in self.r[l]:
            if self.grid_type[l][i, j] == 1 and (i + j) & 1 == phase:
