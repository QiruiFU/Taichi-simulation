...
@ti.data_oriented
class Newton:
    def __init__(self, energy_fn, grad_fn, hess_fn, DBC_fn=None,dim=3, alpha=0.5, beta=0.6, eta=1e-3, grad_normalizer=1.0):
        self.dim = dim
        ...
        self.d = ti.ndarray(float, shape=dim)
        self.f0 =0.0

        # self.h_eps_builder = ti.linalg.SparseMatrixBuilder(dim, dim, max_num_triplets=dim,dtype=float)
    ...
    def line_search(self):
        ...
        @ti.kernel
        def calc_g0(d:ti.types.ndarray()) -> float:
            g = 0.0
        ...
        @ti.kernel
        def update_temp(a: float, d :ti.types.ndarray()):
            for i in range(self.dim):
        ...
        return alpha
    # @ti.kernel
    # def init_eps(self,h_eps_builder:ti.types.sparse_matrix_builder(),eps:float):
    #     for i in range(self.dim):
    ...
    def minimize(self, max_iter=200, init_iter=50):
        ...
        for it in range(max_iter):
            ...
            # 更新参数
            @ti.kernel
            def update_x(a: float, d: ti.types.ndarray()):
                for i in range(self.dim):
                    self.x[i] += a * d[i]
