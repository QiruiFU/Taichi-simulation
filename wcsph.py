...
numParInCell = ti.field(int, shape=int(numCell))
# how many particles in this cell
# usage: numParInCell[cellID]
neighbor = ti.field(int, shape=(int(numPar), int(maxNumNeighbors)))
...
cell2Par = ti.field(int, shape=(int(numCell), int(maxNumParInCell)))
    ...
    for i in range(numPar):
        ...
        for ii in ti.static(range(9)):
            ...
            if IsInBound(cellToCheck):
                ...
                for k in range(numParInCell[int(cellToCheck)]):
                    # kth particle in this cell

                    j = cell2Par[int(cellToCheck), int(k)]
